heatmapPlot <- function(matList, sigMat=NULL, qnorm=NULL, tnorm=NULL,
    ...
    if(!is.null(sigMat) && (min(sigMat) < 0 || max(sigMat) > 1))
        stop('sigMat has to range in [0,1] ...')
    if(!is.null(qnorm) && (!is.numeric(qnorm) ||
                           length(qnorm) != length(matList)))
        stop('qnorm has to be either NULL or an array of numeric having
        the same length as matList ...')
    if(!is.null(tnorm) && (!is.numeric(tnorm) ||
                           length(tnorm) != length(matList)))
        stop('tnorm has to be either NULL or of an array of numeric having
            the same length as matList ...')
    if(length(qnorm) > 1 && length(tnorm) > 1 &&
       length(intersect(which(is.null(qnorm)), which(is.null(tnorm)))) > 0)
   ...
   # normalization
    ...
    if(!is.null(tnorm)) {
        for(i in 1:length(tnorm)) {
            if(is.null(tnorm[i])) next
            matList[[i]][matList[[i]] > tnorm[[i]]] <- tnorm[[i]]
            matList[[i]] <- matList[[i]] / tnorm[[i]]
        }
    }

    # normalizing to 1 (dividing by the maximum) the matList entries
    # not normalized with qnorm or tnorm
    notNorm <- which(is.null(qnorm) & is.null(tnorm))
    if(length(notNorm) > 0) {
        for(i in 1:length(matList)) {
            if(max(matList[[i]], na.rm=TRUE) > 1) {
                matList[[i]] <- matList[[i]]/max(matList[[i]], na.rm=TRUE)
                matList[[i]][matList[[i]] > 1] <- 1
            }
        }
    }

   # assembling the matrix for the heatmap
    mat <- NULL
   ...
   # clustering
    if(!is.null(clusterInds)) {
        ...
        NAinds <- which(NAcounts > ncol(mat) * 0.30)
        if(length(NAinds) == nrow(mat)) return(NULL)
        if(length(NAinds) > 0) { # all NA rows are discarded
            mat <- mat[-NAinds,]
            clmat <- clmat[-NAinds,]
        }
        clRes <- hclust(dist(clmat), method = "average")
        rowv <- as.dendrogram(hclust(dist(clmat), method = "average"))
        if(dendrogram) dendr <- 'row'
        else dendr <- 'none'
    }
    else {
        rowv <- FALSE
    ...
    else collab <- colLab; cmar <- 4
    if(is.null(margins)) margins <- c(cmar,rmar)

    # plotting
    if(!is.null(sigMat)) {
        # 30 levels for intensity and 10 levels for significance
        colmat <- palette2d(n=30, k=10)
        cols <- as.vector(t(colmat))
        colbreaks <- 0:300
        sigMat[sigMat < 1e-10] <- 1e-10
        scaledSig <- round(-log10(sigMat))
        scaledSig[scaledSig > 0] <- scaledSig[scaledSig > 0]- 1
        scaledSigMat <- NULL
        nbins <- ncol(matList[[1]])
        for(i in 1:ncol(scaledSig)) {
            for(j in 1:nbins) scaledSigMat <- cbind(scaledSigMat, scaledSig[,i])
        }
        mat <- round(1 + mat*29)
    ...
    res <- heatmap.2(x=mat, breaks=colbreaks, Colv=FALSE, Rowv=rowv, dendrogram=dendr,
             scale="none", col=cols, density.info="none", symkey=FALSE,
             trace="none", margins=margins, labRow=rowlab, labCol=collab,
